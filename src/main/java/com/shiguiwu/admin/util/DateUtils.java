...
public class DateUtils {
    ...
    /**
     * 日期格式，年月日，例如：20050630，2008/08/08
     */
     ...
     * @return
     */
    public static List<String> getDateByWeek(String startDateStr, String endDateStr, Integer... weeks) {
        List<String> listDate = Lists.newArrayList();
        List<Integer> list = Arrays.asList(weeks);
        // 定义一些变量
        Date beginDate = null;
        Date endDate = null;
        ...
        try {
            // 将字符串parse成日期
            beginDate = f.parse(startDateStr);
            endDate = f.parse(endDateStr);
            ...
            while (beginGC.getTime().compareTo(endGC.getTime()) <= 0) {
                ...
                Integer i = getDayWeekOfDate2(beginGC.getTime());
                if (list.contains(i)) {
                    listDate.add(f.format(beginGC.getTime()));
                    // 以日为单位，增加时间
                }
                beginGC.add(Calendar.DAY_OF_MONTH, 1);
            }
    ...
    public static List<String> getStartDate(String startDatestr, String endDateStr){
        List<String> listDate = Lists.newArrayList();

        SimpleDateFormat f = new SimpleDateFormat("yyyy-MM-dd");

        String date = StringUtils.substring(startDatestr, 8);
        ...
        try {
            ...
            while (beginGC.getTime().compareTo(endGC.getTime()) <= 0) {
                ...
                String tmp = parseDateToStr(beginGC.getTime(), DATE_FORMAT_YYYY_MM_DD);
                String a = StringUtils.substring(tmp, 8);
                System.out.println(tmp);
                flag = true;
                while (!date.equalsIgnoreCase(a)) {
                    beginGC.add(Calendar.DATE, 1);//增加一天
                    tmp = parseDateToStr(beginGC.getTime(), DATE_FORMAT_YYYY_MM_DD);
                    a = StringUtils.substring(tmp, 8);
                    if(flag) {
                        beginGC.add(Calendar.MONTH, 1);
                        flag = false;
...
//
//        Integer integer = 2;
////        System.out.println(Objects.equals(2, integer));
//        Calendar cal = Calendar.getInstance();
//
...
//        Date date = parseStrToDate("2019-10-11", DateUtils.DATE_FORMAT_YY_MM_DD);
//        tt(list);
//        System.out.println(list.size());
//
//        String dateStr = "2019-08-02";
//        String dateStr1 = "2019-08";



//        Date date = parseStrToDate(dateStr1, DATE_FORMAT_YYYY_MM_DD);
//        System.out.println(StringUtils.substring(dateStr, 0, 7));

//        List<String> endDate = getStartDate("2019-01-30", "2019-07-17");
